...
RejectQuoteAction::RejectQuoteAction(AppCtx* appCtx, shared_ptr<Session> session) : appCtx(appCtx), session(session) {
}

RejectQuoteAction::~RejectQuoteAction() {
	BOOST_LOG_TRIVIAL(info) << "RejectQuoteAction:delete:start";
	BOOST_LOG_TRIVIAL(info) << "RejectQuoteAction:delete:end";
}

void RejectQuoteAction::doAction(unique_ptr<Document> rq, string& ss){
	Document res;
	Document::AllocatorType& alloc = res.GetAllocator();
	Value hdr(kObjectType);
	Value bdy(kObjectType);
	Value type("reject_quote_result");
	...
	conv->status = Status::REJECT_QUOTE;
	
	dao->quote(conv);
	appCtx->getConversationManager()->sendConversationEvent(conv);

	BOOST_LOG_TRIVIAL(info) << "RejectQuote:" << conv->id;
