...
RejectOrderAction::RejectOrderAction(AppCtx* appCtx, shared_ptr<Session> session) : appCtx(appCtx), session(session) {
}

RejectOrderAction::~RejectOrderAction() {
	BOOST_LOG_TRIVIAL(info) << "RejectOrderAction:delete:start";
	BOOST_LOG_TRIVIAL(info) << "RejectOrderAction:delete:end";
}

void RejectOrderAction::doAction(unique_ptr<Document> rq, string& ss){
	Document res;
	...
	conv->status = Status::REJECT_ORDER;
