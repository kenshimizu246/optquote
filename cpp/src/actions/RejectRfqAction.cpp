...
RejectRfqAction::RejectRfqAction(AppCtx* appCtx, shared_ptr<Session> session) : appCtx(appCtx), session(session) {
}

RejectRfqAction::~RejectRfqAction() {
	BOOST_LOG_TRIVIAL(info) << "RejectRfqAction:delete:start";
	BOOST_LOG_TRIVIAL(info) << "RejectRfqAction:delete:end";
}

void RejectRfqAction::doAction(unique_ptr<Document> rq, string& ss){
	Document res;
	Document::AllocatorType& alloc = res.GetAllocator();
	Value hdr(kObjectType);
	Value bdy(kObjectType);
	Value type("reject_rfq_result");
