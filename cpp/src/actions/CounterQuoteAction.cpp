...
#include "../Leg.hpp"
#include "CounterQuoteAction.hpp"
...
CounterQuoteAction::CounterQuoteAction(AppCtx* appCtx, shared_ptr<Session> session) : appCtx(appCtx), session(session) {
}

CounterQuoteAction::~CounterQuoteAction() {
	BOOST_LOG_TRIVIAL(info) << "CounterQuoteAction:delete:start";
	BOOST_LOG_TRIVIAL(info) << "CounterQuoteAction:delete:end";
}

void CounterQuoteAction::doAction(unique_ptr<Document> rq, string& ss){
	Document res;
	Document::AllocatorType& alloc = res.GetAllocator();
	Value hdr(kObjectType);
	Value bdy(kObjectType);
	Value type("counter_quote");
